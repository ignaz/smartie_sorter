...
/****** FSM conditions ***************************************************
*************************************************************************/
static uint8_t cond_true(void)         {   return 1;   }
static uint8_t cond_else(void)         {   return 1;   }
static uint8_t cond_conveyor_idle(void){   return MC_Is_Conveyor_Idle(); }
static uint8_t cond_catcher_idle(void) {   return MC_Is_Catcher_Idle();  }
static uint8_t cond_md_init(void)      {   return (cur_mode == md_init)    ? 1 : 0 ;}
static uint8_t cond_md_run(void)       {   return (cur_mode == md_running) ? 1 : 0 ;}
static uint8_t cond_md_learn(void)     {   return (cur_mode == md_learning)? 1 : 0 ;}
static uint8_t cond_md_not_learn(void) {   return (cur_mode != md_learning)? 1 : 0 ;}
static uint8_t cond_md_pause(void)     {   return (cur_mode == md_pause)   ? 1 : 0 ;}
    ...
    uint16_t temp_ui16 = 0;
    {
        switch (state)
        {
            ...
            case st_init_cs:
                ...
                CS_LED_Addapt(&cs_sensor_data,CS_MAX_VAL);
                MC_Conveyor_Set_Position(+1);
                break;
